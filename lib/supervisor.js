...
//
// `start({ bots })` is not a hole in that, and the difference is worth stating precisely
// rather than waving at. A caller does not supply an argument: it selects between two
// argument lists this file already holds, and `!== false` forces the selection, so a string,
// an object or whatever a query string produced lands on one of them and nothing else. What
// must never happen is a caller's VALUE reaching argv, and none does. It belongs on `start`
// rather than on the supervisor because it is a property of the session being recorded — the
// grid changes between races — where `out` and `udpPort` are properties of the machine.
//
...
function createSupervisor(options) {
  const opts = options || {};
  const out = opts.out;
  // `script: null` means the command IS the recorder — a release exe, which carries its own
  // runtime and takes the flags directly. `undefined` still means the default script, so a
  // caller that passes neither gets what it always got.
  const script = opts.script === null ? null : (opts.script || RECORD);
  ...
  /** `opts.bots === false` records the people on the grid and skips the AI. */
  function start(opts) {
    if (child) return { code: 'already-recording', error: 'A recording is already running.' };

    const bots = !(opts && opts.bots === false);
    const args = script ? [script, '--supervised', '--out', out] : ['--supervised', '--out', out];
    if (udpPort) args.push('--udp-port', String(udpPort));
    if (!bots) args.push('--no-bots');
    ...
    // `bots` is published from here rather than waited for off the child, so a page that
    // asked for a filtered recording shows that it got one straight away instead of a second
    // later. The child says the same thing in its own events, and it is the same value.
    publish({ running: true, state: 'starting', pid: proc.pid, bots });
    return { ok: true, pid: proc.pid, bots };
  }
