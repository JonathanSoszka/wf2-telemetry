...
//
// ONE AT A TIME. Two recorders polling the same telemetry both decide a session has started
// and both open a file, so a single race becomes two recordings of it, each missing whatever
// the other's poll happened to win. There is no useful reason to run two, so starting one
// while another is alive is refused rather than queued.
//
...
//
// THE CHILD'S ARGUMENTS ARE NOT THE CALLER'S. Only the source is selectable, and the output
// directory is always the one the server was started with. Passing request parameters
// through to a spawned process is how a browsable local tool becomes an arbitrary-file-write
// endpoint for whatever else the browser is running.
//
...
// The set of sources the child actually accepts, taken from the child rather than restated
// here: a whitelist that drifts from it either rejects a valid source or forwards an
// invalid one to a process that exits on it. Requiring record.js does not run it — it only
// calls main() when it is the entry point.
const { SOURCES } = require('../record.js');
...
function createSupervisor(options) {
  ...
  // Where SimHub is. Configured on the server, never in a request — which SimHub to poll is
  // a property of the machine, and letting a caller name a host and port would turn this
  // into a way to make the user's own machine issue requests anywhere.
  ...
  function start(request) {
    ...
    const requested = request && request.source;
    const source = SOURCES.includes(requested) ? requested : 'api';
    const args = [script, '--supervised', '--source', source, '--out', out];
    ...
    // The recorder puts real trouble on stderr — SimHub unreachable, a port taken. Only the
    // tail is kept: it retries forever, so the same complaint can repeat indefinitely.
    ...
    const finish = (state, extra) => {
      ...
      child = null;
      stopping = null;
      publish(Object.assign({ running: false, state, source, stderr: stderr.trim() || undefined }, extra));
    };
    ...
    publish({ running: true, state: 'starting', source, pid: proc.pid });
